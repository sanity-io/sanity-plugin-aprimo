...
export const RenditionSelector = ({renditions, changeRendition, currentRendition}: RenditionProps) => {
	const placeholder = <option value="" disabled selected hidden>Select rendition...</option>
	...
	if (currentRendition && renditions) {
		const foundRendition = renditions.find((rendition: Record<string, any>) => rendition.id == currentRendition.id)
		if (foundRendition) { selectedRendition = foundRendition }
	}
